...
func Test_logsHandlerDoesNotLeakGoroutinesWhenProviderClosesStream(t *testing.T) {
	...
	querier := newFiniteQueryRequestor(msgs, nil)
...
type finiteQueryRequestor struct {
	Logs   []Message
	err    error
	stream chan Message
}

func (r finiteQueryRequestor) Close() {
	close(r.stream)
	r.stream = nil
}

func (r finiteQueryRequestor) Query(context.Context, Request) (<-chan Message, error) {
	if r.err != nil {
		return nil, r.err
	}
...
func newFiniteQueryRequestor(l []Message, err error) finiteQueryRequestor {
	return finiteQueryRequestor{
		Logs:   l,
		err:    err,
